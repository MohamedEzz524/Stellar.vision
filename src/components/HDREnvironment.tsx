...
export const HDREnvironment = ({
  ...
  useEffect(() => {
    ...
    const setupEnvironmentMap = (texture: THREE.Texture) => {
      ...
      finalTexture.offset.x = rotationY;
      finalTexture.offset.y = rotationX;
      ...
      if (!isHDR && !isEXR && intensity !== 1.0) {
        ...
        if (ctx && img && img instanceof HTMLImageElement) {
          ...
          adjustedTexture.offset.x = finalTexture.offset.x;
          adjustedTexture.offset.y = finalTexture.offset.y;
      ...
      try {
        ...
        // If PMREM conversion fails, use texture directly (rotation already applied)
